...
- [ ] 検索・ページネーション・オフラインキャッシュの実機確認（2026-08-08 実装。未検証。
  特にオフラインは機内モードで一度開いた範囲を開き直し、灰バナーとキャッシュ表示を確認すること）
- [ ] フロートのスクロールボタン・明細編集メモ欄のキーボード回避の実機確認（2026-08-08 実装。未検証）
- [ ] **検索欄の入力継続確認**（2026-08-09。ヘッダー再マウントで1文字ごとにキーボードが
  閉じる不具合を修正。上の「検索」項目は動作OKの報告があったが、直った後の状態を
  改めて確認すること）
- [ ] デモモードの表示確認（未送信一覧・「前回の登録」がデモ中に出ないこと）、
  一括処理中に再サインインが必要になった場合の挙動、明細編集の日時欄の検証メッセージ
  （2026-08-09 実装。いずれも未検証）
...
- **新しい月シートの初回作成時、ヘッダー行の書き込みが競合すると直後の追記が消えうる**
  （`SheetsService.ensureSheet`/`execWrite`）。`addSheet` 後にヘッダーを A1 へ無条件 PUT する
  処理と、別プロセス（夫婦の別端末、または同一端末の Gmail 自動取込と固定費月初コピーの
  並行実行）がほぼ同時に同じ新規月シートへ最初の1行を書こうとした場合、後から確定する
  ヘッダー PUT が先に入っていた明細行（A1:L1）を上書きしうる。月初・月をまたいだ直後に
  夫婦が同時に最初の支出を記録する、という限定的だが起こりうる条件。Sheets API に
  トランザクションが無いため、根本対応には「書き込み前に A1 が空か確認してからヘッダーを
  書く」等の緩和策の検討が要る。
- **固定費の月初自動コピーが端末をまたいで二重作成されうる**（`applyRecurringEntries`）。
  ...
  2つ作りうる。発生条件が狭い（月初にほぼ同時起動）ため優先度は低いが、Sheets 側に
  重複排除の仕組みが無い以上、根本対応は難しい。
- **Gemini 呼び出しの最悪ケース待機時間に上限・キャンセル手段が無い**（`CameraScreen`/
  `GeminiProvider`）。429 が複数モデルで連続すると理論上 7 分半ほどブロックしうる。
  UI は「OCR解析中...」の表示のみでキャンセル導線が無い。上限時間の設定やキャンセル
  ボタンの追加を検討の余地あり。
