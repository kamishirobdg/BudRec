...
最終更新: 2026-08-02 / 最新コミット `f34883a`（Windows ローカルに未 push のコミットが 4 つある）
...
│   ├── CameraScreen.tsx     (927行) レシート撮影・ギャラリー選択・OCR→確認→保存
│   ├── SummaryScreen.tsx    (1464行) 一覧/集計画面 ＋ 明細編集モーダル ★中核
...
- **通信エラーへの耐性**: 429/5xx の自動リトライ、送れなかった書き込みの退避と自動再送（下記参照）
...
- 保存前の確認画面は無い（単票のときと同じ）。誤読は一覧画面で編集・削除する。
- OCR 失敗時の手動入力モーダルは 1 件ぶんのみ。複数枚を手入力したいときは個別に撮り直す。

**精度の注意**: 並べる枚数が増えるほど 1 枚あたりの解像度が落ちる。撮影は `quality: 0.6`
（ギャラリー選択は 0.8）なので、読めない場合は枚数を減らすか、`CameraScreen.handleShoot` の
quality を上げる。**実機での精度は未検証**（何枚まで実用になるかはこれから確かめる）。
...
- **退避された追加行は、送信されるまで一覧に出ない**（行番号がサーバー側で決まるため）。
  バナーの文言でその旨を伝えている。
...
- [ ] リトライ / 未送信キュー / ErrorBoundary の実機確認（機内モードで撮影 → 復帰で自動送信されるか）
- [ ] 複数レシート一括読み取りの実機確認（**何枚並べても読めるか。精度が要検証**）
...
- [ ] 読み込み失敗時の再試行 UI（現状は Alert のみ。引き下げ更新で再取得はできる）
- [ ] 未送信の追加行を一覧に「送信待ち」として重ねて表示する（今は送信されるまで見えない）
...
```
f34883a feat: 送れなかった書き込みを端末に退避して後で自動送信する  ← 現在の HEAD（未 push）
41a6f5f feat: ErrorBoundaryを追加してクラッシュ時の白画面を回避する      （未 push）
be03884 feat: Sheets/Gmailの429・5xx・通信断を指数バックオフで自動リトライ（未 push）
24b82de docs: 恒久キーストアは見送りの決定を記録                         （未 push）
