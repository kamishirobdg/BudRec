...
最終更新: 2026-09-13 / `claude/reduce-google-relogin-MCMj8` の最新コミット `9a0679f`
＋ モーダル内のキーボード隠れ対策（本コミット）
...
`_settings`（ユーザー/カテゴリ）,
`_config`（既定一部計上額・Gmail検索期間・固定費を月初コピー済みの月）,
...
- 明細編集モーダル（取込元・ユーザーは読み取り専用表示）。RN の `Modal` は Android では
  ...
  包んで対応（2026-08-08）
  - **同じ穴が他の 2 つのモーダルにも残っていたので塞いだ（2026-09-13）。**
    `ReceiptReviewModal`（複数レシートの確認）と `CameraScreen` の手動入力モーダル。
    件数が多いほど下のカードが深くなるため、**複数登録時に金額・メモがキーボードに
    隠れて見えない**という形で出ていた。`KeyboardAvoidingView`（`behavior="height"`）＋
    `ScrollView` に `flex: 1` で、キーボードぶんだけ表示領域が縮むようにしてある。
    縮めば ScrollView の可視範囲から外れるので、Android 側がフォーカス中の入力欄を
    自動でスクロールして見せてくれる（縮まないと「画面内にある」と判断され動かない）。
  - **`Modal` の中に入力欄を置くときは、必ずこの対策が要る。** 新しいモーダルを足すときは
    `KeyboardAvoidingView` で包むこと。`ReceiptReviewModal` ではフッター（登録ボタン）も
    中に入れて、キーボードが出ている間はその上に載るようにしてある。
...
2. `PreferencesService.getRecurringAppliedMonth()`（**端末ローカル**）が当月と一致すれば
   何もしない。これは「起動のたびに通信しない」ための目印にすぎない
3. `SheetsService.applyRecurringEntries()` が **`_config` の `recurring_applied_month`**（共有）を見る。
   当月と一致すれば他端末が済ませているので 0 を返す。**こちらが正**
4. 一致しなければ、**コピーする前に**その値を当月にして書く（先に確保する）。
   失敗したら消してから投げ直す（残すと今月ぶんが永久に作られない）
5. **前月シート**の `recurring && !deleted` 行を当月へコピー
...
6. 重複防止キー = `store|category|user|amount`（当月の `source === 'recurring'` 行から構築）
7. 作成件数 > 0 なら一覧を再読み込み

**フラグを端末ローカルではなく共有に置く理由**は下記「固定費の月初コピーを端末間で
二重に作らない」を参照（2 台が月初にほぼ同時に起動すると二重に作られていた）。
...
> **現在は導線を畳んである（2026-08-29）。** 利用期間が終わったため
> `DemoService.DEMO_MODE_AVAILABLE = false` にしてあり、設定画面の ON/OFF ボタンと
> 設定パネルは表示されない。**機能そのものは一切削っていない**ので、また外部に見せる
> 機会が来たら**このフラグを true に戻すだけ**で以下がそのまま復活する。
>
> - フラグが false の間は `loadConfig()` が `enabled` を強制的に false にする。
>   導線が無い状態でデモに入ったままだと解除する手段が無くなるため。
>   **Storage の保存内容は書き換えない**ので、表示名対応表・カテゴリ別合計・倍率などの
>   設定は残ったままで、フラグを戻せば以前の状態で使える。
> - 設定画面の読み込みでも、デモ専用の `getUniqueUsersRaw()` / `getRowsRaw()` を呼ばなくなる
>   （導線が無いのに Sheets を余分に読まないため）。
> - `SheetsService` / `GmailService` / `SummaryScreen` などの `isDemo()` 分岐は残してある。
>   フラグが false の間は常に「デモではない」経路を通るだけで、動作は変わらない。

以下はフラグを戻したときのふるまい（実装は現存する）。
...
### 動作確認待ち
- [ ] **モーダル内のキーボード隠れ対策（2026-09-13 修正。未検証）**。複数レシートを読み取って
  確認モーダルを開き、**いちばん下のカードの金額・メモをタップして、入力中の文字が
  キーボードに隠れずに見えること**。登録ボタンがキーボードの上に出ること。
  OCR 失敗時の手動入力モーダルでも同様に確認する
...
- [ ] **異常系だけ未確認**（通常操作では踏まないため上記の確認では通っていない）:
  ...
  - ~~デモ ON のまま未送信キューが残っているときに設定画面・一覧に実データが出ないか~~
    → **デモの導線を畳んだので当面は踏めない**（2026-08-29）。`DEMO_MODE_AVAILABLE` を
    true に戻して再開するときに確認する
...
- [ ] 固定費の月初コピー（2026-08-12 修正。未検証）。判断が `_config` 側に移ったので、
  **通常経路（月が変わって初回起動したとき固定費が 1 セットだけ作られる）が要確認**。
  `_config` に `recurring_applied_month` の行ができることも見ておく
- [ ] 未送信キューと直接書き込みの整合（2026-08-12 実装。未検証）。機内モードで
  ある行の除外をトグル → 通信を戻す → **送信せずに**同じ行のメモを編集して保存 →
  未送信バナーから送信 → **除外もメモも両方残っている**ことを確認する
  （直っていないと、後から流れたキューがメモを消す or 除外が元に戻る）
...
### 未送信キューと直接書き込みの整合（2026-08-12 実装）

キューの項目は「積んだ時点のスナップショット」を持つ。通信が戻ったあとに同じ行を編集して
**その書き込みだけ先に成功**すると、あとから流れる古いキュー項目が新しい内容を上書きしていた。
原因は順序だけでなく **書き込みの入力そのものが古いこと** でもあったので、2 か所を直した。

**1. 成功した直接書き込みに合わせてキューを畳む**（`WriteQueueService.reconcileAfterDirectWrite`）

各操作が書く列は決まっている: `updateRow`=A:L / `updateFlags`=H:J / `setRecurring`=K /
`markDeleted`=L。`writeOrQueue` が成功したら、同じ `sheetName+rowIndex` のキュー項目から
**重なる列だけ**を潰す。

- 全部の列が上書きされたキュー項目は捨てる（例: 直接 `updateRow` 成功 → 同じ行の
  `updateFlags` は不要）。
- 部分的にしか重ならないのは `updateRow` だけ（A:L をまとめて書くので列を削れない）。
  この場合は捨てずに、**重なる列の値だけ新しいものに差し替えて**送る。
  例: オフラインでメモを直して `updateRow` が退避 → 復帰後に除外をトグルして
  `updateFlags` が直接成功 → キューの `updateRow` の H:J だけ新しい値に差し替える。
  こうするとメモ（オフラインの編集）も除外（オンラインの編集）も両方残る。
- 単純に「その行のキューを全部捨てる」にしなかったのはこのため。捨てるとオフラインで
  直したメモが消える。
- **キューからの再送（`flushWriteQueue`）では呼ばないこと。** キュー内は積んだ順に送るので
  畳む必要が無く、畳むとむしろ壊れる。

**2. 「前回の登録」がシートを読み直す際に未送信の変更を重ねる**（`WriteQueueService.applyPendingTo`）

`handleOpenLastBatch` は表示中の期間と関係なくシートを読み直すため、退避中の変更が
反映されていなかった。そこから保存すると **書き込む中身が既に古い** ので、1 だけでは
上書きを防げない（キューを畳んでも、畳んだ先の値が古い）。読み込んだ行に未送信の変更を
重ねてからモーダルに渡すようにした。

- `pickBatchRows` に `overlay` 引数を足し、**シート上の値と未送信を重ねた値のどちらでも
  照合する**（返すのは重ねたほう）。キーがどちらの値で記録されているかは経路で違うため:
  一覧の編集モーダル経由の未送信はキーが古いまま、「前回の登録」経由の未送信は
  保存時に `saveLastBatch` が走るのでキーが新しくなっている。
- 一覧画面自体は楽観的更新で `rows` が既に最新なので、重ねる必要は無い。

**対象外**: 別端末（夫婦のもう一方）が同じ行を直していた場合の競合。検知には行ごとの
バージョン管理と読み込み後の書き戻しが要り、Sheets 相手では過剰と判断した。
**同一端末内の整合性に絞っている。**

### 固定費の月初コピーを端末間で二重に作らない（2026-08-12 実装）

「適用済み月」の判断を **端末ローカルからスプレッドシートの `_config` へ移した**
（`CONFIG_KEY_RECURRING_APPLIED_MONTH`）。端末ごとに持っていたときは、夫婦の 2 台が
月初にほぼ同時に起動すると両方が「まだ未適用」と判断し、同じ固定費行を 2 つ作りえた。

- **コピーする前に `_config` へ書く（先に確保する）。** 済ませてから書くと、その間に
  起動したもう一方の端末も未適用と判断してしまい、窓が「読み込み＋N 行の追記」ぶん開く。
  先に書けば窓は 1 往復ぶんに縮む。
- 途中で失敗したらフラグを消してから投げ直す。残すと今月ぶんが永久に作られなくなるため。
- 既存の重複防止キー（`store|category|user|amount`）はそのまま残してある。確保の直後に
  当月シートを読み直すので、わずかな差で先を越された場合はこちらが拾う。
- `PreferencesService` の端末ローカルのフラグは残してあるが、**役割は「起動のたびに
  通信しない」ための目印だけ**に降格した。正は `_config` 側。

**残る穴**: 確保した直後にアプリが強制終了されると、フラグだけ立って行が作られない
（あるいは途中まで）状態になり、その月は自動で作り直されない。Sheets API に CAS が無く、
これ以上は排他制御を持ち込むことになるため許容している。気づいたら手で足すこと。

### コード整理（2026-08-12）

`<documentDirectory>/*.json` の読み書きを `services/jsonFileStore.ts` に集約した
（`readJsonArray` / `writeJson` / `removeFile` / `nowLabel`）。
`WriteQueueService` / `RowsCacheService` / `LastBatchService` の 3 つが
「JSON を 1 ファイルに置き、壊れていたら諦めて空として扱う」という同じ処理を
それぞれ写経していて、片方だけ直す事故が起きやすかったため。
