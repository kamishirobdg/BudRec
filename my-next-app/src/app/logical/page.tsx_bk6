...
function shortPlanet(p: string) {
  // purely visual; keep the long label available in tooltip
  // (You asked for the planet name itself, so we keep it as text; this is only used if you later prefer abbreviations.)
  return p;
}
...
export default function LogicalMapPage() {
  ...
  const svg = useMemo(() => {
    ...
    const segKey = (s: Seg) => {
      // order-insensitive key to dedupe shared edges between tiles (should not happen for border edges, but keep safe)
    ...
    return (
      ...
      >
        ...
        {entries.map(([k, cell]) => {
          ...
          // 表示文字：惑星は planetType、非惑星で scout center のみ Scout
      ...
      {result ? (
        <>
          <div style={{ fontSize: 12, opacity: 0.85 }}>
            <span>
              templateId=<code>{String(result.templateId)}</code>, seed=<code>{String(result.seed)}</code>, cells=
              <code>{String(result.cellsByKey?.size ?? 0)}</code>, collisions=<code>{String(result.collisionCount ?? 0)}</code>
            </span>
          </div>

          <h2 style={{ fontSize: 14, fontWeight: 700 }}>Fine Board (Rendered – flat-top default)</h2>

          {/* toast overlay (non-intrusive) */}
