...
function computeGAIABonusByType(
  ...
  for (const [k, c] of cells.entries()) {
    if (c.kind === "planet" && c.planet === "GAIA") {
      const a = parseKey(k);
        if (!Number.isFinite((a as any)?.q) || !Number.isFinite((a as any)?.r)) continue;
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        ...
        <div style={{ width: "100%", height: "80vh", overflow: "hidden", border: "1px solid #eee", borderRadius: 10, marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {board.cells.size > 0 && renderMode === "cells" && (
            <svg viewBox={`${boardVb.x} ${boardVb.y} ${boardVb.w} ${boardVb.h}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ display: "block" }}>
              {keys.map((k) => {
