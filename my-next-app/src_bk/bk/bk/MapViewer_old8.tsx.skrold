...
// --- Thresholds ---
  ...
  const bounds = useMemo(() => {
    // For tile-image mode, the rendered extents are larger than per-cell bounds.
    // Compute bounds from placements + image half-size so the SVG viewBox fits and centers correctly.
    if (renderMode === "tiles") {
      if (!activeTemplate) return { minX: 0, minY: 0, maxX: 0, maxY: 0 };
      ...
      for (const p of effectivePlacements) {
        ...
        minX = Math.min(minX, x - imgHalf);
        minY = Math.min(minY, y - imgHalf);
        maxX = Math.max(maxX, x + imgHalf);
        maxY = Math.max(maxY, y + imgHalf);
      }
      if (!isFinite(minX)) return { minX: 0, minY: 0, maxX: 0, maxY: 0 };
      return { minX, minY, maxX, maxY };
    }

    // Default (cells) bounds
    ...
    return { minX, minY, maxX, maxY };
  }, [keys, size, renderMode, tileImgFactor, effectivePlacements, activeTemplate]);
