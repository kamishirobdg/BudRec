...
import { EXPANSION_TILES_ALL } from "../GAIA/sectorTiles_expansion";
...
export default function MapViewer() {
  ...
  const SECTORS_WITH_TILE_TAG = useMemo(() => {
    ...
    // Expansion tiles are sparse (footprint: tri3/single). Keep sparse structure and only tag cells.
    const exp = (EXPANSION_TILES_ALL as any[]).map((t) => ({
      ...t,
      cells: (t.cells ?? []).map((sc: any) => ({
        ...sc,
        cell: {
          ...sc.cell,
          tags: Array.from(new Set([...(sc.cell?.tags ?? []), `tile:${t.id}`])),
        },
      })),
    }));
...
// --- Thresholds ---
  ...
  const assignValidation = useMemo(() => {
    ...
    for (let i = 0; i < assigns.length; i++) bySlotIndex[i] = assigns[i]?.sectorId;
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        ...
        <div style={{ width: "100%", height: "75vh", overflow: "hidden", border: "1px solid #eee", borderRadius: 10, marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {board.cells.size > 0 && renderMode === "cells" && (
            <svg viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ display: "block" }}>
              {keys.map((k) => {
        ...
        const { x, y } = axialToPixel(viewPos, size, ORIENTATION);
                ...
                return (
                  <g key={i} transform={`translate(${x}, ${y}) rotate(${deg}) translate(${-imgSize / 2}, ${-imgSize / 2})`}>
                    <image href={`/sectors/${p.sectorId}.png`} width={imgSize} height={imgSize} preserveAspectRatio="xMidYMid meet" pointerEvents="none" />
                  </g>
                );
              })}
            </svg>
