...
type TryResult =
  | { ok: true; assigns: SlotAssignment[]; attemptsUsed: number; total: number; balanceL1: number }
  | { ok: false; reason: string; attemptsUsed: number; total?: number; balanceL1?: number };
...
export default function MapViewer() {
  ...
  // --- Scoring / Balance thresholds ---
  // 偏り（平均からの乖離）を抑えるための主閾値
  const [balanceMaxL1, setBalanceMaxL1] = useState<number>(3.5);
  // 総スコアの最低限（任意。不要なら -999999 などに）
  const [totalMin, setTotalMin] = useState<number>(-999999);

  // --- Calibration (auto threshold from samples) ---
  const [calibPercentile, setCalibPercentile] = useState<number>(70); // default: stricter than 80
  const [calibTargetValid, setCalibTargetValid] = useState<number>(400);
  const [calibMaxAttemptsCap, setCalibMaxAttemptsCap] = useState<number>(8000);
  const [calibMode, setCalibMode] = useState<"no-dup" | "rot-only">("no-dup");
  const [calibStatus, setCalibStatus] = useState<string>("");
  const [calibRunning, setCalibRunning] = useState<boolean>(false);
  const [calibLastSummary, setCalibLastSummary] = useState<{
    tplId: string;
    mode: "no-dup" | "rot-only";
    percentile: number;
    targetValid: number;
    attemptsUsed: number;
    validCollected: number;
    l1Min: number;
    l1P50: number;
    l1Px: number;
    l1P90: number;
  ...
  const scoreResult = useMemo(() => {
    if (board.cells.size === 0) return null;
  ...
  // ---------------------------
  // Randomize（偏り抑制：balance.l1）
  // ---------------------------
  function tryBuildWithAssigns(
    tpl: MapLayoutTemplate,
    cand: SlotAssignment[],
    thresholds: { balanceMaxL1: number; totalMin: number },
    ...
    try {
      ...
      const s = scoreBoard(b.cells, { cellScoreMap });
      const total = s.total;
      const balanceL1 = s.balance.l1;

      if (total < thresholds.totalMin) {
        return { ok: false, reason: `Total rejected: total=${total} < totalMin=${thresholds.totalMin}`, total, balanceL1 };
      }
      if (balanceL1 > thresholds.balanceMaxL1) {
        return {
          ok: false,
          reason: `Balance rejected: L1=${balanceL1.toFixed(3)} > max=${thresholds.balanceMaxL1}`,
          total,
          balanceL1,
        };
      }

      return { ok: true, assigns: cand, total, balanceL1 };
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
  ...
  function pushHistory(assignsOk: SlotAssignment[], attemptsUsed: number, total: number, balanceL1: number) {
    const id = `${Date.now()}-${Math.random().toString(16).slWHITE(2)}`;
    const item: HistoryItem = { id, atIso: isoNow(), attemptsUsed, total, balanceL1, assigns: assignsOk };
  ...
  function applyHistory(item: HistoryItem) {
    ...
    setRandomizeStatus(
      `Applied history: ${shortIso(item.atIso)} (attempts=${item.attemptsUsed}, total=${item.total}, L1=${item.balanceL1.toFixed(3)})`
    );
  }
  ...
  function randomizeNoDupAndValidate() {
    ...
    const thresholds = { balanceMaxL1, totalMin };
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      const res = tryBuildWithAssigns(activeTemplate, cand, thresholds, cellScoreMap);
      if (res.ok) {
        setAssigns(res.assigns);
        setLastAttempts(attempt);
        setRandomizeStatus(`Randomize OK (attempt ${attempt}/${MAX_ATTEMPTS}) total=${res.total} L1=${res.balanceL1.toFixed(3)}`);
        pushHistory(res.assigns, attempt, res.total, res.balanceL1);
        return;
      } else {
    ...
    setRandomizeStatus(
      `Randomize failed (no solution in ${MAX_ATTEMPTS}). Last: ${lastReason}${typeof lastTotal === "number" ? ` (total=${lastTotal})` : ""}${
  ...
  function randomizeRotOnlyAndValidate() {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (res.ok) {
        setAssigns(res.assigns);
        setLastAttempts(attempt);
        setRandomizeStatus(`Randomize rot OK (attempt ${attempt}/${MAX_ATTEMPTS}) total=${res.total} L1=${res.balanceL1.toFixed(3)}`);
        pushHistory(res.assigns, attempt, res.total, res.balanceL1);
        return;
      } else {
        lastReason = res.reason;
    ...
    setLastAttempts(MAX_ATTEMPTS);
    setRandomizeStatus(
      `Randomize rot failed (no solution in ${MAX_ATTEMPTS}). Last: ${lastReason}${typeof lastTotal === "number" ? ` (total=${lastTotal})` : ""}${
        typeof lastL1 === "number" ? ` (L1=${lastL1.toFixed(3)})` : ""
      }`
    );
  }
  ...
  // ---------------------------
  // Calibration（L1 distribution -> set balanceMaxL1 to Px）
  // ---------------------------
  async function calibrateBalanceThreshold() {
    if (!activeTemplate) {
      setCalibStatus("No active template.");
      return;
    }
    ...
    const targetValid = Math.max(50, Math.floor(calibTargetValid));
    const maxAttempts = Math.max(targetValid, Math.floor(calibMaxAttemptsCap));
    const px = Math.min(99, Math.max(1, calibPercentile));

    setCalibRunning(true);
    setCalibStatus(`Calibrating... targetValid=${targetValid}, maxAttempts=${maxAttempts}, Px=P${px}, mode=${calibMode}`);

    const l1s: number[] = [];
    let attemptsUsed = 0;

    // rot-only uses current sectorId sequence (no-dup assumed already)
    const baseIds = assigns.slWHITE(0, slotCount).map((a, i) => (a?.sectorId ?? defaultAssign(i).sectorId) as SectorId);
    ...
    try {
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (calibMode === "no-dup") {
          ...
          cand = baseIds.map((id) => ({ sectorId: id, rot: randInt(0, 5) }));
        }

        // build + distance constraint only (NO balance threshold during sampling)
        try {
          const eff = placementsFromTemplate(activeTemplate, cand);
          const b = buildLogicBoardWithLocalColumnShift(BASE_SECTORS, eff, {
            viewRot60: VIEW_ROT60,
            viewMirror: VIEW_MIRROR,
            orientationForColumnOrder: "flat",
          ...
          if (v.length > 0) {
            ...
            const s = scoreBoard(b.cells, { cellScoreMap });
            l1s.push(s.balance.l1);
            if (l1s.length >= targetValid) break;
          }
        } catch {
          // skip build errors in sampling
        }

        // yield to UI periodically (avoid long blocking)
        if (attempt % 250 === 0) {
          setCalibStatus(`Calibrating... valid=${l1s.length}/${targetValid}, attempts=${attempt}/${maxAttempts}`);
          // eslint-disable-next-line no-await-in-loop
          await new Promise((r) => setTimeout(r, 0));
        }
      }

      if (l1s.length < Math.min(50, targetValid)) {
        setCalibStatus(`Calibration failed: too few valid samples (valid=${l1s.length}). Try increasing maxAttempts or relax constraints.`);
        setCalibRunning(false);
        return;
      }

      const sorted = [...l1s].sort((a, b) => a - b);
      const l1Min = sorted[0];
      const l1P50 = percentileOfSorted(sorted, 50);
      const l1Px = percentileOfSorted(sorted, px);
      const l1P90 = percentileOfSorted(sorted, 90);
      const l1Max = sorted[sorted.length - 1];

      // IMPORTANT: accept if L1 <= balanceMaxL1, so set threshold to Px of L1 (lower Px => stricter)
      setBalanceMaxL1(Number(l1Px.toFixed(4)));

      setCalibLastSummary({
        tplId: activeTemplate.id,
        mode: calibMode,
        percentile: px,
        targetValid,
        attemptsUsed,
        validCollected: l1s.length,
        l1Min,
        l1P50,
        l1Px,
        l1P90,
      ...
      setCalibStatus(
        `Calibration OK: set balanceMaxL1 = P${px}(L1) = ${l1Px.toFixed(4)} (valid=${l1s.length}, attempts=${attemptsUsed})`
      );
    } finally {
      setCalibRunning(false);
    }
    ...
    <div style={{ display: "grid", gridTemplateColumns: "460px 1fr", gap: 16, padding: 16 }}>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
          ...
          <div style={{ marginTop: 6, border: "1px solid #eee", borderRadius: 10, padding: 10 }}>
            <div style={{ fontWeight: 800, marginBottom: 6 }}>Scoring balance (7 types, avg-based)</div>

            <label style={{ display: "grid", gap: 4 }}>
              balanceMaxL1 (reject if L1 &gt; max)
              <input
                type="number"
                step="0.1"
                value={balanceMaxL1}
            ...
            <label style={{ display: "grid", gap: 4, marginTop: 8 }}>
              totalMin (reject if total &lt; min) — optional
              <input
                type="number"
                value={totalMin}
            ...
            <div style={{ marginTop: 10, borderTop: "1px solid #eee", paddingTop: 10 }}>
              <div style={{ fontWeight: 800, marginBottom: 6 }}>Auto-calibrate balanceMaxL1 from samples</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <label style={{ display: "grid", gap: 4 }}>
                  ...
                  >
                    <option value="no-dup">no-dup (sectorId+rot)</option>
                    <option value="rot-only">rot-only (keep sectorId)</option>
                  </select>
                </label>
                ...
                <label style={{ display: "grid", gap: 4 }}>
                  percentile (Px of L1)
                  <select
                    value={calibPercentile}
                    onChange={(e) => setCalibPercentile(Number(e.target.value))}
                    style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
                    disabled={calibRunning}
                  >
                ...
                <label style={{ display: "grid", gap: 4 }}>
                  ...
                  <input
                    type="number"
                    value={calibTargetValid}
                    onChange={(e) => setCalibTargetValid(Number(e.target.value))}
                    style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
                    disabled={calibRunning}
                  />
                </label>
                ...
                <label style={{ display: "grid", gap: 4 }}>
                  ...
                  <input
                    type="number"
                    value={calibMaxAttemptsCap}
                    onChange={(e) => setCalibMaxAttemptsCap(Number(e.target.value))}
                    style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
                    disabled={calibRunning}
                  />
                </label>
              ...
              <button
                onClick={calibrateBalanceThreshold}
                disabled={!activeTemplate || calibRunning}
                style={{
                  marginTop: 10,
                  width: "100%",
                  ...
                  padding: "10px 12px",
                  background: "#fff",
                  cursor: activeTemplate && !calibRunning ? "pointer" : "not-allowed",
                  opacity: activeTemplate && !calibRunning ? 1 : 0.5,
                }}
              >
                Calibrate & set balanceMaxL1
              </button>
              ...
              {calibLastSummary && (
                <div style={{ marginTop: 8, fontSize: 12, color: "#333", border: "1px solid #f0f0f0", borderRadius: 10, padding: 8, background: "#fff" }}>
                  <div style={{ fontWeight: 800, marginBottom: 6 }}>Calibration summary</div>
                  <div style={{ fontFamily: "ui-monospace, Menlo, monospace" }}>
                    tpl={calibLastSummary.tplId}
                    <br />
                    mode={calibLastSummary.mode} Px=P{calibLastSummary.percentile} targetValid={calibLastSummary.targetValid}
                    <br />
                    valid={calibLastSummary.validCollected} attempts={calibLastSummary.attemptsUsed}
                    <br />
                    L1: min={calibLastSummary.l1Min.toFixed(4)} p50={calibLastSummary.l1P50.toFixed(4)} px={calibLastSummary.l1Px.toFixed(4)} p90={calibLastSummary.l1P90.toFixed(4)} max=
                    {calibLastSummary.l1Max.toFixed(4)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ marginTop: 10, fontSize: 12, color: "#333" }}>
              {scoreResult ? (
                <>
                  total: <b>{scoreResult.total}</b> / min: <b>{totalMin}</b>
                  <br />
                  balance L1: <b>{scoreResult.balance.l1.toFixed(3)}</b> / max: <b>{balanceMaxL1}</b>
                  <br />
                  meanAvg: <b>{scoreResult.balance.meanAvg.toFixed(3)}</b> / maxAbs: <b>{scoreResult.balance.maxAbs.toFixed(3)}</b>
                  ...
                  <div style={{ marginTop: 8, borderTop: "1px solid #eee", paddingTop: 8 }}>
                    <div style={{ fontWeight: 800, marginBottom: 6 }}>Per-type (avg / total / count)</div>
                    <div style={{ display: "grid", gap: 4 }}>
                      {scoreResult.balance.includedTypes.map((t) => (
                        <div key={t} style={{ display: "flex", justifyContent: "space-between", fontFamily: "ui-monospace, Menlo, monospace", fontSize: 12 }}>
                          <span>{t}</span>
                          <span>
                            avg={scoreResult.perType.avgs[t].toFixed(3)} total={scoreResult.perType.totals[t].toFixed(1)} n={scoreResult.perType.counts[t]}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ color: "#666" }}>(no board)</div>
              )}
            </div>
          </div>
                  ...
                  <div key={h.id} style={{ border: "1px solid #f0f0f0", borderRadius: 10, padding: 8 }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <div style={{ fontFamily: "ui-monospace, Menlo, monospace", fontSize: 12 }}>
                        attempts={h.attemptsUsed} total={h.total} L1={h.balanceL1.toFixed(3)}
                      </div>
                    </div>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
          <div style={{ fontWeight: 900 }}>Fine Board (Rendered) — hover to see target, click to select</div>
        ...
        <div style={{ width: "100%", height: "75vh", overflow: "auto", border: "1px solid #eee", borderRadius: 10, marginTop: 8 }}>
          {board.cells.size === 0 ? (
            <div style={{ padding: 12, color: "#666" }}>No render (build error)</div>
          ) : (
