...
import { EXPANSION_MIDDLE_IDS, EXPANSION_LITTLE_IDS, EXPANSION_SCOUT_IDS, EXPANSION_TILES_ALL } from "../GAIA/sectorTiles_expansion";
...
export default function MapViewer() {
  ...
  const SECTORS_WITH_TILE_TAG = useMemo(() => {
    ...
    // Expansion tiles are defined as sparse footprints (MIDDLE=tri3, SMALL=single).
    // buildLogicBoardWithLocalColumnShift only needs id + (q,r)->Cell mapping, so we adapt them here.
    const exp = (EXPANSION_TILES_ALL as any[]).map((t) => {
      const cells: Record<string, any> = {};
      for (const sc of t.cells ?? []) {
        const key = `${sc.at.q},${sc.at.r}`;
        const c = sc.cell;
        cells[key] = {
          ...c,
          tags: Array.from(new Set([...(c.tags ?? []), `tile:${t.id}`])),
        };
      }
      return {
        id: t.id,
        radius: 1, // tri3/single are safely embedded in radius=1 space
        cells,
      };
    });
