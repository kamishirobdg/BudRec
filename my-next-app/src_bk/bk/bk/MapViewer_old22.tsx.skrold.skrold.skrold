...
import { checkBaseMinus2Cap, formatBaseMinus2CapViolation } from "../GAIA/baseMinus2Cap";
...
type SlotConstraint3P =
  | "NONE"
  | "ALL_147_IN_01_04"
  | "AT_LEAST2_OF_147_IN_01_04"
  | "SLOT4_IN_01_04_AND_SLOT1_OR7_IN_01_04"
  | "SLOT4_MUST_IN_01_04";

const SECTOR_SET_01_04 = new Set<string>(["01", "02", "03", "04"]);

function isSectorIn01to04(sectorId: string): boolean {
  const id = normalizeSectorIdForAssetsAndLogic(sectorId);
  return SECTOR_SET_01_04.has(id);
}

function check3pSlotConstraint(mode: SlotConstraint3P, cand: SlotAssignment[]): { ok: true } | { ok: false; reason: string } {
  if (mode === "NONE") return { ok: true };

  // 3P slots: slot1/4/7 => index 0 / 3 / 6 (1-indexed: 1,4,7)
  const s1 = cand[0]?.sectorId ?? "";
  const s4 = cand[3]?.sectorId ?? "";
  const s7 = cand[6]?.sectorId ?? "";

  const in1 = isSectorIn01to04(s1);
  const in4 = isSectorIn01to04(s4);
  const in7 = isSectorIn01to04(s7);

  if (mode === "ALL_147_IN_01_04") {
    if (in1 && in4 && in7) return { ok: true };
    return { ok: false, reason: "3P slot constraint: slots 1/4/7 must all be sectors 01–04." };
  }

  if (mode === "AT_LEAST2_OF_147_IN_01_04") {
    const c = Number(in1) + Number(in4) + Number(in7);
    if (c >= 2) return { ok: true };
    return { ok: false, reason: "3P slot constraint: at least 2 of slots 1/4/7 must be sectors 01–04." };
  }

  if (mode === "SLOT4_IN_01_04_AND_SLOT1_OR7_IN_01_04") {
    if (in4 && (in1 || in7)) return { ok: true };
    return { ok: false, reason: "3P slot constraint: slot4 must be 01–04, and either slot1 or slot7 must be 01–04." };
  }

  if (mode === "SLOT4_MUST_IN_01_04") {
    if (in4) return { ok: true };
    return { ok: false, reason: "3P slot constraint: slot4 must be sectors 01–04." };
  }
...
/**
 * Fast path: compute only Base (tileId -> points) by PlanetType.
 * This intentionally skips GAIA/GAIA/EMPTY and does not compute Neighbor / GAIA bonuses.
 * Used to enforce 3P Base(-2) cap before running full Score Audit.
 */
function computeBaseByTileByTypeOnly(
  cells: Map<string, Cell>,
  cellScoreMap: DraftScoreMap | null
): Record<string, Record<string, number>> {
  const out: Record<string, Record<string, number>> = {};

  // cells entries keys are already "q,r"
...
function defaultAssignFromPool(i: number): SlotAssignment {
  return { sectorId: SECTOR_IDS_BASE[i % SECTOR_IDS_BASE.length], rot: 0 };
}
...
function computeScoreAndBalance(
  ...
  opts: {
    ...
    includeTRANSDIMInNeighbor: boolean;
  ...
  neighborByTileByType: Record<string, Record<string, { n: number; types: Set<string>; points: number }>>;
  ...
  const neighborByTileByType: Record<string, Record<string, { n: number; types: Set<string>; points: number }>> = {};
  ...
  const shouldCountNeighborPlanet = (key: string, cell: Cell): boolean => {
    ...
    if (!opts.includeTRANSDIMInNeighbor && cell.planet === "TRANSDIM") return false;
  ...
  for (const [ak, ac] of cells.entries()) {
    ...
    if (points !== 0) {
      ...
      const row = neighborByTileByType[anchorType][anchorTileId] ?? { n: 0, types: new Set<string>(), points: 0 };
      row.n += n;
...
export default function MapViewer() {
  ...
  const [tileRotDir] = useState<1 | -1>(1);
  const [tileRotOffset60] = useState<number>(2);
  const [tileRotOffsetDeg] = useState<number>(0);


  // Global rotation applied to the whole tile map (60° steps). Use this to match the "cells" orientation perfectly.
  const [tileMapRot60] = useState<number>(0);
// --- Thresholds ---
  ...
  // 3P slot constraint (selectable)
  const [slotConstraint3p, setSlotConstraint3p] = useState<SlotConstraint3P>("NONE");

  // 3P: reject candidates if per-PlanetType count of BaseTile points == -2 exceeds this cap (default 2).
  const [baseMinus2Cap3p, setBaseMinus2Cap3p] = useState<number>(2);
  const [baseMinus2CapViolation, setBaseMinus2CapViolation] = useState<string>("");
  ...
  const [includeTRANSDIMInNeighbor, setIncludeTRANSDIMInNeighbor] = useState<boolean>(true);
  ...
  const scoreResult = useMemo(() => {
    ...
    return computeScoreAndBalance(board.cells, cellScoreMap, {
      ...
      neighborScoreMode,
      includeTRANSDIMInNeighbor,
      neighborRequireNonNegativeCellScore,
      neighborWCount,
      neighborWType,
    });
  }, [board, effectiveCellScoreMap, enableGAIADist2, enableGAIADist1Extra, neighborScoreMode, includeTRANSDIMInNeighbor, neighborRequireNonNegativeCellScore, neighborWCount, neighborWType]);

  
  useEffect(() => {
    if (!activeTemplate || activeTemplate.tags.players !== 3) {
      setBaseMinus2CapViolation("");
      return;
    }
    const s: any = scoreResult as any;
    const baseByTileByType: any = s?.baseByTileByType ?? {};
    const cap = baseMinus2Cap3p;
    const res = checkBaseMinus2Cap(baseByTileByType, cap);
    if (!res.ok) setBaseMinus2CapViolation(formatBaseMinus2CapViolation(res, cap));
    else setBaseMinus2CapViolation("");
  }, [activeTemplate, scoreResult, baseMinus2Cap3p]);
const currentTotal = useMemo(() => {
  ...
  // Fast pre-checks for speed
  if (tpl.tags.players === 3) {
    const slotCheck = check3pSlotConstraint(slotConstraint3p, cand);
    ...
    try {
      ...
      if (v.length > 0) {
        ...
        return { ok: false, reason: `Constraint violation: ${top.tag} ${top.aKey}↔${top.bKey} (d=${top.dist})` };
            ...
            // 3P Base(-2) cap (fast path before full Score Audit)
      if (tpl.tags.players === 3 && baseMinus2Cap3p !== null && baseMinus2Cap3p !== undefined) {
        const baseByTileByType = computeBaseByTileByTypeOnly(b.cells, cellScoreMap ?? null);
        const capRes = checkBaseMinus2Cap(baseByTileByType as any, baseMinus2Cap3p);
        if (!capRes.ok) {
          return {
            ok: false,
            reason: formatBaseMinus2CapViolation(capRes, baseMinus2Cap3p),
          };
        }
      }
...
const s: any = computeScoreAndBalance(b.cells, cellScoreMap ?? null, {
  ...
  function randomizeRotOnlyAndValidate() {
    ...
    const pool = getSectorPoolForTemplate(activeTemplate);
    const baseIds = assigns.slWHITE(0, slotCount).map((a, i) => (a?.sectorId ?? defaultAssignFromPool(i, pool).sectorId) as SectorId);

    // 3P slot constraint pre-check: sector ids are fixed in rot-only mode
    if (activeTemplate.tags.players === 3) {
      const dummyCand: SlotAssignment[] = baseIds.map((id, i) => ({ sectorId: id, rot: 0 }));
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
          ...
          <div style={{ marginTop: 6, border: "1px solid #eee", borderRadius: 10, padding: 10 }}>
            ...
            <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px solid #eee" }}>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>3P Slot constraint</div>

              <label style={{ display: "grid", gap: 4 }}>
                slot constraint preset (applies only when players=3)
                <select
                  value={slotConstraint3p}
                  onChange={(e) => setSlotConstraint3p(e.target.value as SlotConstraint3P)}
                  style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
                >
                  <option value="NONE">No constraint</option>
                  <option value="ALL_147_IN_01_04">slot1/4/7 all in sectors 01–04</option>
                  <option value="AT_LEAST2_OF_147_IN_01_04">at least 2 of slot1/4/7 in sectors 01–04</option>
                  <option value="SLOT4_IN_01_04_AND_SLOT1_OR7_IN_01_04">slot4 in 01–04 and (slot1 or slot7) in 01–04</option>
                  <option value="SLOT4_MUST_IN_01_04">slot4 must be in sectors 01–04</option>
                </select>
              </label>

              <div style={{ marginTop: 6, fontSize: 12, color: "#333" }}>
                Note: slots are 1-indexed; we interpret slot1/4/7 as indWHITEs 0/3/6.
              </div>
            </div>

            {/* 3P Base(-2) cap */}
            <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px solid #eee" }}>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>3P Base(-2) cap</div>

              <label style={{ display: "grid", gap: 4 }}>
                cap (reject if per-PlanetType count of BaseTile points == -2 exceeds cap) — default 2
                <input
                  type="number"
                  step="1"
                  min={0}
                  value={baseMinus2Cap3p}
                  onChange={(e) => setBaseMinus2Cap3p(Math.max(0, Math.floor(Number(e.target.value))))}
                  style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
                />
              </label>

              {baseMinus2CapViolation ? (
                <div style={{ marginTop: 6, fontSize: 12, color: "#b00020" }}>{baseMinus2CapViolation}</div>
              ) : (
                <div style={{ marginTop: 6, fontSize: 12, color: "#333" }}>OK</div>
              )}
            </div>
              ...
              <>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, tableLayout: "fixed" }}>
                    <thead>
                      <tr style={{ background: "#fafafa" }}>
                        <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eee" }}>PlanetType</th>
                        <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #eee" }}>Base</th>
                        <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #eee" }}>GAIA</th>
                        <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #eee" }}>Neighbor</th>
                        <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #eee" }}>Total</th>
                        <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eee", whiteSpace: "normal", wordBreak: "break-word" }}>Base tiles (non-zero)</th>
                        <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eee", whiteSpace: "normal", wordBreak: "break-word" }}>GAIA tiles</th>
                      </tr>
                    </thead>
                    <tbody>
                      {scoreAuditRows.map((r) => (
                        ...
                        >
                          <td style={{ padding: 8, borderBottom: "1px solid #f3f3f3", fontWeight: 700 }}>{r.pt}</td>
                          <td style={{ padding: 8, borderBottom: "1px solid #f3f3f3", textAlign: "right" }}>{r.baseSum}</td>
                          <td style={{ padding: 8, borderBottom: "1px solid #f3f3f3", textAlign: "right" }}>{r.GAIASum}</td>
                          <td style={{ padding: 8, borderBottom: "1px solid #f3f3f3", textAlign: "right" }}>{r.neighborSum}</td>
                          <td style={{ padding: 8, borderBottom: "1px solid #f3f3f3", textAlign: "right", fontWeight: 800 }}>{r.total}</td>
                          <td
                            title={r.baseTilesStr || ""}
                            style={{
                              padding: 8,
                ...
                {(() => {
                  ...
                  const neighborEntries = Object.entries(neighborByTile)
                    .map(([tid, v]) => ({ tid, n: v.n, u: (v.types ? Array.from(v.types).length : 0), types: v.types ? Array.from(v.types).sort() : [], points: v.points }))
                      ...
                      <div style={{ display: "grid", gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: "#333", marginBottom: 6 }}>
                            mode: <b>{neighborScoreMode}</b> / include TRANSDIM: <b>{String(includeTRANSDIMInNeighbor)}</b> / require cellScore&gt;=0:{" "}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <select
                value={tileRotDir}
                onChange={(e) => _setTileRotDir(Number(e.target.value) as 1 | -1)}
                style={{ marginLeft: 6 }}
              >
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                min={-6}
                max={6}
                value={tileRotOffset60}
                onChange={(e) => _setTileRotOffset60(Number(e.target.value))}
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                min={-30}
                max={30}
                value={tileRotOffsetDeg}
                onChange={(e) => _setTileRotOffsetDeg(Number(e.target.value))}
                style={{ marginLeft: 6, width: 56 }}
              />
            </label>
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                max={6}
                value={tileMapRot60}
                onChange={(e) => _setTileMapRot60(Number(e.target.value))}
                style={{ marginLeft: 6, width: 56 }}
              />
            </label>
          </div>
