...
function defaultAssignFromPool(i: number, pool: readonly string[] = SECTOR_IDS_BASE): SlotAssignment {
  return { sectorId: pool[i % pool.length], rot: 0 };
}
...
// --- Thresholds ---
  ...
  // --- Base -2 cap (3p default: 2) ---
  // For each PlanetType, if count of Base tile contributions that equal -2 exceeds this cap, the candidate is rejected.
  // Stored per template; default applies when players=3 and no saved value exists.
  const [baseMinus2Cap, setBaseMinus2Cap] = useState<number | null>(null);
  const [baseMinus2CapMsg, setBaseMinus2CapMsg] = useState<string>("");
  ...
  useEffect(() => {
    ...
    // default policy:
    // - 3p templates: baseMinus2Cap defaults to 2 (unless user saved a value)
    // - others: null (disabled) by default
    const is3p = activeTemplate?.tags?.players === 3;

    if (!st) {
      setBaseMinus2Cap(is3p ? 2 : null);
      setBaseMinus2CapMsg("");
      return;
    }
    ...
    if (typeof st.baseMinus2Cap === "number") setBaseMinus2Cap(st.baseMinus2Cap);
    else if (st.baseMinus2Cap === null) setBaseMinus2Cap(null);
    else setBaseMinus2Cap(is3p ? 2 : null);
  ...
  useEffect(() => {
    if (!activeTemplateId) return;
    saveTemplateSettings(activeTemplateId, { balanceMaxL1, scoreThreshold, baseMinus2Cap });
  }, [activeTemplateId, balanceMaxL1, scoreThreshold, baseMinus2Cap]);
  ...
  function tryBuildWithAssigns(
    ...
    try {
      ...
      // Base(-2) cap (3p only)
      if (tpl.tags.players === 3 && typeof baseMinus2Cap === "number") {
        const capRes = checkBaseMinus2Cap(s.baseByTileByType ?? {}, baseMinus2Cap);
        if (!capRes.ok) {
          const msg = formatBaseMinus2CapViolation(capRes, baseMinus2Cap);
          return { ok: false, reason: msg, scoreTotal: total, balanceL1: typeof s.balanceL1 === "number" ? s.balanceL1 : undefined };
        }
      }
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12, overflowX: "hidden", maxWidth: "100%", minWidth: 0 }}>
        ...
        <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
          ...
          <div style={{ marginTop: 6, border: "1px solid #eee", borderRadius: 10, padding: 10 }}>
            ...
            <label style={{ display: "grid", gap: 4, marginTop: 8 }}>
              Base(-2) cap per PlanetType (3p default=2; blank=disable)
              <input
                type="number"
                min={0}
                step={1}
                value={baseMinus2Cap ?? (activeTemplate?.tags.players === 3 ? 2 : "")}
                placeholder={activeTemplate?.tags.players === 3 ? "2" : "disabled"}
                onChange={(e) => {
                  const raw = e.target.value.trim();
                  if (raw === "") {
                    setBaseMinus2Cap(null);
                    setBaseMinus2CapMsg("");
                    return;
                  }
                  const n = Number(raw);
                  if (!Number.isFinite(n)) return;
                  setBaseMinus2Cap(Math.max(0, Math.floor(n)));
                  setBaseMinus2CapMsg("");
                }}
                disabled={activeTemplate?.tags.players !== 3}
                style={{ padding: 6, borderRadius: 8, border: "1px solid #ccc" }}
              />
            </label>

            {baseMinus2CapMsg && (
              <div style={{ marginTop: 6, fontSize: 12, color: "#b00020" }}>{baseMinus2CapMsg}</div>
            )}
