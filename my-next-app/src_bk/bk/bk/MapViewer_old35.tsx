...
import { EXPANSION_SECTORS, EXPANSION_MIDDLE_IDS } from "../GAIA/sectorTiles_expansion";
import { BASE_SECTOR_ID_POOL, EXP_SECTOR_ID_POOL } from "../GAIA/sectorPools";
...
function isMiddleSectorId(id: string): boolean {
  return EXPANSION_MIDDLE_IDS.includes(id as any);
}
...
function getSectorPoolForTemplate(tpl: MapLayoutTemplate | null): readonly string[] {
  const is3p = tpl?.tags.players === 3;
  const isExp = tpl?.tags.expansion === "exp";

  // 既存の「3P固定プール」思想は維持しつつ、expならMiddle候補も末尾に足す
  if (is3p) {
    const base = SECTOR_IDS_3P_FIXED;
    return isExp ? ([...base, ...EXPANSION_MIDDLE_IDS] as const) : base;
  }

  // 4P/その他
  return isExp ? EXP_SECTOR_ID_POOL : BASE_SECTOR_ID_POOL;
}


const ALL_SECTOR_DEFS = [...BASE_SECTORS, ...EXPANSION_SECTORS] as const;

function hasSectorDef(sectorId: string): boolean {
  return ALL_SECTOR_DEFS.some((s) => s.id === sectorId);
}
...
{effectivePlacements.map((p, i) => {
  ...
  const viewPos = rotate60(
    p.pos,
    (VIEW_ROT60 + tileMapRot60 + 600) % 6
  );
  const { x, y } = axialToPixel(viewPos, size, ORIENTATION);

  const deg =
    ((VIEW_ROT60 + tileMapRot60 + p.rot + 600) % 6) * 60;

  // ★ ここで分岐
  if (isMiddleSectorId(sectorId)) {
    // ← Middle 用描画（前回提示した box 計算 + <image>）
  }

  // ← 既存の通常セクター描画（imgSize / 正方形）
}}
            </svg>
          )}
