...
function computeGAIABonusByType(cells: Map<string, Cell>, radius: number): Record<string, number> {
  const GAIAs: { q: number; r: number }[] = [];
  for (const [k, c] of cells.entries()) {
    if (c.kind === "planet" && c.planet === "GAIA") GAIAs.push(parseKey(k));
  }
  const bonusByType: Record<string, number> = {};
  for (const t of SCORE_TYPES) bonusByType[t] = 0;

  if (GAIAs.length === 0) return bonusByType;
  ...
  for (const [k, c] of cells.entries()) {
    ...
    for (const s of GAIAs) {
      if (axialDist(a, s) <= radius) cnt += 1;
    }
    if (cnt > 0) bonusByType[pt] += cnt;
  }
...
function computeScoreAndBalance(
  ...
  GAIARadius: number
): { total: number; balanceL1: number; totalByType: Record<string, number>; GAIABonusByType: Record<string, number> } {
  const baseByType: Record<string, number> = {};
  for (const t of SCORE_TYPES) baseByType[t] = 0;
  ...
  const GAIABonusByType = computeGAIABonusByType(cells, GAIARadius);
  const totalByType: Record<string, number> = {};
  for (const t of SCORE_TYPES) totalByType[t] = (baseByType[t] ?? 0) + (GAIABonusByType[t] ?? 0);

  const vals = SCORE_TYPES.map((t) => totalByType[t] ?? 0);
  const mean = vals.reduce((s, v) => s + v, 0) / vals.length;

  // Normalized L1 deviation: (mean absolute deviation) / (average absolute magnitude).
  ...
  return { total, balanceL1, totalByType, GAIABonusByType };
}
...
// --- Thresholds ---
  ...
  const { board, buildError } = useMemo(() => {
    try {
      const b = buildLogicBoardWithLocalColumnShift(BASE_SECTORS, logicPlacements, {
  ...
  const scoreResult = useMemo(() => {
    ...
    return computeScoreAndBalance(board.cells, cellScoreMap, 2);
  ...
  function tryBuildWithAssigns(
    ...
    try {
      ...
      const b = buildLogicBoardWithLocalColumnShift(BASE_SECTORS, effLogic, {
        viewRot60: VIEW_ROT60,
        viewMirror: VIEW_MIRROR,
        orientationForColumnOrder: "flat",
        sizeForColumnOrder: 28,
      ...
      const s: any = computeScoreAndBalance(b.cells, cellScoreMap ?? null, 2);
