...
function computeGAIABonusByType(
  ...
  opts: { enableDist2: boolean; enableDist1Extra: boolean; scoreTypes: PlanetType[] }
  ...
  for (const t of opts.scoreTypes) {
    bonusByType[t] = 0;
  ...
  for (const [k, c] of cells.entries()) {
    ...
    if (!opts.scoreTypes.includes(pt)) continue;
  ...
  const GAIATilesByType: Record<string, string[]> = {};
  for (const t of opts.scoreTypes) GAIATilesByType[t] = Array.from(GAIATilesSetByType[t]).sort();
...
function computeScoreAndBalance(
  ...
  opts: {
    ...
    includeTRANSDIMInPlanetTypeBase: boolean;
  ...
  const baseTypes: PlanetType[] = opts.includeTRANSDIMInPlanetTypeBase ? [...SCORE_TYPES, "TRANSDIM"] : SCORE_TYPES;
  ...
  for (const [k, c] of cells.entries()) {
    if (c.kind !== "planet") continue;
    const pt = c.planet as PlanetType;
    if (!baseTypes.includes(pt)) continue;
  ...
  const vals = baseTypes.map((t) => totalByType[t] ?? 0);
...
export default function MapViewer() {
  ...
  const [最大試行回数, setMaxAttempts] = useState<number>(0); // 0 = 無制限

  // 3人戦 Slot制約 (5-choWHITE)
...
// --- Thresholds ---
  ...
  const [includeTRANSDIMInNeighbor,
      includeTRANSDIMInPlanetTypeBase,
      includeTRANSDIMInPlanetTypeBase, setIncludeTRANSDIMInNeighbor] = useState<boolean>(true);
  const [includeTRANSDIMInPlanetTypeBase, setIncludeTRANSDIMInPlanetTypeBase] = useState<boolean>(false);
  ...
  const scoreTypesBase: PlanetType[] = includeTRANSDIMInPlanetTypeBase ? [...SCORE_TYPES, "TRANSDIM"] : SCORE_TYPES;
  ...
  const scoreAuditRows = useMemo(() => {
    ...
    const rows = scoreTypesBase.map((pt) => {
  ...
  function randomizeNoDupAndValidate() {
    ...
    (async () => {
      for (let attempt = 1; ; attempt++) {
        ...
        if (最大試行回数 > 0 && attempt > 最大試行回数) {
          setIsSearching(false);
          setLastAttempts(最大試行回数);
          setRandomizeStatus(`Search failed (no solution in ${最大試行回数}). Last reason: ${lastRejectReason || "(none)"}`);
          return;
        }
          ...
          if (activeTemplate?.tags?.players === 3) {
            ...
            if (!sc.ok) {
              ...
              if (attempt % 200 === 0) {
                ...
                setSearchElapsedMs(経過);
                setSearchSpeed(経過 > 0 ? (attempt / 経過) * 1000 : 0);
                if (now - lastUiUpdate > 250) {
                  setRandomizeStatus(`Searching… attempts=${attempt} (${((attempt / 経過) * 1000).toFixed(0)}/s)`);
                  lastUiUpdate = now;
                }
                await yieldToUi();
              }
        ...
        if (res.ok) {
          ...
          setRandomizeStatus(
            `Found! attempts=${attempt} (${(経過 / 1000).toFixed(2)}s, ${((attempt / 経過) * 1000).toFixed(0)}/s) total=${res.scoreTotal}${
    ...
    (async () => {
      for (let attempt = 1; ; attempt++) {
        ...
        if (res.ok) {
          ...
          setRandomizeStatus(
            `Found! (rot only) attempts=${attempt} (${(経過 / 1000).toFixed(2)}s, ${((attempt / 経過) * 1000).toFixed(0)}/s) total=${res.scoreTotal}${
              typeof res.balanceL1 === "number" ? ` L1=${res.balanceL1.toFixed(3)}` : ""
            }`
          );
          pushHistory(res.assigns, attempt, res.scoreTotal, res.balanceL1);
        ...
        if (attempt % 200 === 0) {
          ...
          setSearchSpeed(経過 > 0 ? (attempt / 経過) * 1000 : 0);
          if (now - lastUiUpdate > 250) {
            setRandomizeStatus(`Searching (rot only)… attempts=${attempt} (${((attempt / 経過) * 1000).toFixed(0)}/s)`);
            lastUiUpdate = now;
          }
          await yieldToUi();
        }
...
{activeTemplate?.tags?.players === 3 ? (
  ...
  <div style={{ marginTop: 8 }}>
    <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 4 }}>採点対象（Base/監査）にTRANSDIMを含める</div>
    <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
      <input
        type="radio"
        name="includeTRANSDIMInPlanetTypeBase"
        checked={!includeTRANSDIMInPlanetTypeBase}
        onChange={() => setIncludeTRANSDIMInPlanetTypeBase(false)}
      />
      含めない（従来どおり）
    </label>
    <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
      <input
        ...
        name="includeTRANSDIMInPlanetTypeBase"
        checked={includeTRANSDIMInPlanetTypeBase}
        onChange={() => setIncludeTRANSDIMInPlanetTypeBase(true)}
      />
      含める
    </label>
    <div style={{ fontSize: 11, color: "#666", marginTop: 2 }}>
      ※含める場合、Score Audit（PlanetType別）・balance計算の対象にもTRANSDIMが追加されます
    </div>
  </div>

<div style={{ fontSize: 12, color: "#333", display: "grid", gap: 2 }}>
              <div>
                試行回数（今回）: <b>{lastAttempts}</b>
              </div>
              <div>
                経過: <b>{(searchElapsedMs / 1000).toFixed(2)}s</b> / 速度: <b>{searchElapsedMs > 0 ? searchSpeed.toFixed(0) : 0}/s</b>
              </div>
              {lastRejectReason ? (
                <div style={{ color: "#666" }}>
              ...
              <div style={{ color: "#666" }}>最大試行回数: {最大試行回数 === 0 ? "∞" : 最大試行回数}</div>
            </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <select
                value={tileRotDir}
                onChange={(e) => setTileRotDir(Number(e.target.value) as 1 | -1)}
                style={{ marginLeft: 6 }}
              >
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                onChange={(e) => setTileRotOffset60(Number(e.target.value))}
            ...
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                onChange={(e) => setTileRotOffsetDeg(Number(e.target.value))}
                style={{ marginLeft: 6, width: 56 }}
              />
            </label>
            <label style={{ fontSize: 12 }}>
              ...
              <input
                ...
                max={6}
                value={tileMapRot60}
                onChange={(e) => setTileMapRot60(Number(e.target.value))}
                style={{ marginLeft: 6, width: 56 }}
              />
            </label>
          </div>
