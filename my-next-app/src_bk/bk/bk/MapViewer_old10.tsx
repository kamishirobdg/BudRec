...
export default function MapViewer() {
  ...
  const [tileRotOffset60, setTileRotOffset60] = useState<number>(0);
...
// --- Thresholds ---
  ...
  }, [keys, size, tileImgFactor, effectivePlacements]);
          ...
          {board.cells.size > 0 && renderMode === "tiles" && (
            <svg viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} width={Math.max(900, vb.w)} height={Math.max(650, vb.h)}>
              {effectivePlacements.map((p, i) => {
                ...
                const deg = tileRotDir * ((p.rot + tileRotOffset60 + VIEW_ROT60) * 60) + tileRotOffsetDeg;
