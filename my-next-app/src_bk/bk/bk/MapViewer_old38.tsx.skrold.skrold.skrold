...
import { EXPANSION_MIDDLE_IDS, EXPANSION_LITTLE_IDS, EXPANSION_SCOUT_IDS } from "../GAIA/sectorTiles_expansion";

const EXPANSION_MIDDLE_IDS = ["11a", "11b"] as const;
...
function getSectorPoolForTemplate(tpl: MapLayoutTemplate | null): readonly string[] {
  // Assign UI (no pos) は slotKind を見ないため、テンプレ単位で「選択可能IDプール」を提供する。
  // expansion テンプレでは、SECTOR に加えて MIDDLE/LITTLE/SCOUT の ID も選択できるようにする。
  const isExp = (tpl as any)?.tags?.expansion === "exp";

  if ((tpl as any)?.tags?.players === 3) {
    const base = SECTOR_IDS_3P_FIXED;
    return isExp ? [...base, ...EXPANSION_MIDDLE_IDS, ...EXPANSION_LITTLE_IDS, ...EXPANSION_SCOUT_IDS] : base;
  }

  const base = SECTOR_IDS_BASE;
  return isExp ? [...base, ...EXPANSION_MIDDLE_IDS, ...EXPANSION_LITTLE_IDS, ...EXPANSION_SCOUT_IDS] : base;
}
...
function isMiddleSectorId(id: string): boolean {
  return (EXPANSION_MIDDLE_IDS as readonly string[]).includes(id);
}

const MIDDLE_TRI3_OFFSETS: readonly { q: number; r: number }[] = [
  { q: 0, r: 0 }, // anchor
  { q: 1, r: 0 },
  { q: 0, r: 1 },
] as const;

function computeMiddleImageBox(size: number, orientation: HexOrientation) {
  const pts = MIDDLE_TRI3_OFFSETS.map((a) => axialToPixel(a, size, orientation));
  const xs = pts.map((p) => p.x);
  const ys = pts.map((p) => p.y);

  // pad by one hex radius (roughly) so image has room for outline/whitespace
  const pad = size;
...
const EXPANSION_MIDDLE_SECTOR_DEFS = EXPANSION_MIDDLE_IDS.map((id) => {
  // radius=1 全7セルを empty 定義（validate/logic 対策）
  const t = `tile:${id}`;
  return {
    id,
    radius: 1,
    cells: {
      "0,0": { kind: "empty", tags: [t] },
      "1,0": { kind: "empty", tags: [t] },
      "0,1": { kind: "empty", tags: [t] },
      "-1,1": { kind: "empty", tags: [t] },
      "-1,0": { kind: "empty", tags: [t] },
      "0,-1": { kind: "empty", tags: [t] },
      "1,-1": { kind: "empty", tags: [t] },
    },
...
export default function MapViewer() {
  ...
  const SECTORS_WITH_TILE_TAG = useMemo(() => {
    const ALL_SECTOR_DEFS = [...BASE_SECTORS, ...EXPANSION_MIDDLE_SECTOR_DEFS] as const;

    return ALL_SECTOR_DEFS.map((s) => ({
      ...s,
          ...
          {board.cells.size > 0 && renderMode === "tiles" && (
            <svg viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`} width={Math.max(900, vb.w)} height={Math.max(650, vb.h)}>
              {effectivePlacements.map((p, i) => {
                const sectorId = normalizeSectorIdForAssetsAndLogic(p.sectorId);
                ...
                if (isMiddleSectorId(sectorId)) {
                  const box = computeMiddleImageBox(size, ORIENTATION);
                  return (
                    <g
                      key={`tile-${i}-${sectorId}`}
                      transform={`translate(${x}, ${y}) rotate(${deg}) translate(${box.x}, ${box.y})`}
                      opacity={1}
                      pointerEvents="none"
                    >
                      <image
                        href={`/sectors/${sectorId}.png`}
                        width={box.w}
                        height={box.h}
                ...
                return (
                  <g
                    ...
                    pointerEvents="none"
                  >
                    <image href={`/sectors/${sectorId}.png`} width={imgSize} height={imgSize} preserveAspectRatio="xMidYMid meet" />
                  </g>
                );
              })}</svg>
          )}
