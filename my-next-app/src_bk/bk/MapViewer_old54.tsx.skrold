...
// ===== Middle tile offsets (fixed by rot) =====
// NOTE: The offsets are applied only to MIDDLE tiles in "tiles" render mode.
// rot is the *normalized* rotation used by the logic build (i.e., after the MIDDLE +1 normalization).
...
export default function MapViewer() {
  ...
  // Middle tile offsets are fixed by rotation via MIDDLE_TILE_OFFSETS.
...
const sanitizedTemplateIdsRef = useRef<Set<string>>(new Set());
  ...
  // Rendering must use the same board as the logic build.
  // (Historically, cells-mode applied an extra MIDDLE rotation, which could mask collisions.)
  ...
  const bounds = useMemo(() => {
    ...
    if (effectivePlacements.length > 0) {
      for (const p of effectivePlacements) {
        ...
        // Apply fixed-by-rot offsets only for MIDDLE tiles.
        const rot0 = rot6(Number((p as any).rot) || 0);
        const rotN = isMiddle ? (rot0 + 1) % 6 : rot0;
        const off = isMiddle ? (MIDDLE_TILE_OFFSETS[rotN] ?? { x: 0, y: 0 }) : { x: 0, y: 0 };
        const xx = x + off.x;
        const yy = y + off.y;
  ...
  }, [keys, size, effectivePlacements, tileMapRot60, middleTileImgFactor]);
...
{/* History */}
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 10, padding: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
          ...
          {renderMode === "tiles" && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <div style={{ fontSize: 12, color: "#666" }}>middle offsets: fixed by rot</div>
            </div>
          )}
        ...
        <div style={{ width: "100%", height: "80vh", overflow: "hidden", border: "1px solid #eee", borderRadius: 10, marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
          {renderBoard.cells.size === 0 && <div style={{ padding: 12, color: "#666" }}>No render (build error)</div>}

          {renderBoard.cells.size > 0 && renderMode === "cells" && (
            <svg viewBox={`${safeVb.x} ${safeVb.y} ${safeVb.w} ${safeVb.h}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ display: "block" }}>
              {renderKeys.map((k) => {
                const cell = renderBoard.cells.get(k)!;
                const a = parseKey(k);
        if (!Number.isFinite((a as any)?.q) || !Number.isFinite((a as any)?.r)) return;
                const { x, y } = axialToPixel(a, size, ORIENTATION);
